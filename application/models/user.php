<?php
	...
	* 
	*/
	class User extends CURRV_Model{
		public $table = 'user';
		public $primary_key  = 'user_id';
		public $fillable = array('username','password','user_fname','user_lname','user_mname','user_department','user_position','user_status','user_image','dep_id','user_type_id');
		...
		{
			$this->as_object()->set_cache('get_all_users')->get_all();
		}
		...
		{
			$data = $this->where('username',$username)->as_object()->get();
			if($data){
					if($password == openssl_decrypt($data->password,"AES-128-ECB","CURRV")){
						$data->login =  TRUE;
						$this->session->set_userdata('userSessionLogin', $data);
						redirect(base_url().'gen_info','refresh'); 
					}
			}else{
