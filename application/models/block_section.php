<?php
	...
	* 
	*/
	class Block_section extends CURRV_Model{
		public $table = 'block_section';
		public $primary_key  = 'bs_id';
		public $fillable = array('sec_code','activation','description','year_lvl','semister','sy','pl_id','cur_id');
		...
		public function __construct()
		{
			...
			$this->has_many['schedule'] =  array('sched_subj','bs_id','bs_id');
		...
		public function program($data)
	    {
	        $this->db->select('*')
	            ->from('block_section')
	            ->join('sis_main_db.program_list', 'sis_main_db.program_list.pl_id = block_section.pl_id','left');
	            if(!empty($data['sy']) && !empty($data['semester'])){
	                ...
	                $this->db->where('block_section.bs_id', $data);
	            }
	        $q = $this->db->get();
		...
		{
			$this->as_object()->set_cache('get_all_curr_block_section_list')->get_all();
		}
