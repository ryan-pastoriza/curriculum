<?php
	...
	* 
	*/
	class Sched_day extends CURRV_Model{
		public $table = 'sched_day';
		public $primary_key  = 'sd_id';
		public $fillable = array('abbreviation','composition');
		...
		{
			$this->as_object()->set_cache('get_all_sched_day')->get_all();
		}
		...
		public function getIDByDay($day = '')
		{
			$id = '';
			$days = $this->where('composition',$day)->get_all();
			if(!empty($days)){
				$id  =  $days['sd_id'];
			}
			return $id;
